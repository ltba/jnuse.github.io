...
//随机文字烟花内容
const randomWords = ["知知","我爱你", "好喜欢你呀","菜包♡知知"];
const wordDotsMap = {};
...
function render(speed) {
	...
	// trailsCtx.fillRect(0, 0, width, height); 书签
	const backgroundImage = new Image();
	backgroundImage.src = store.state.config.bg;
	trailsCtx.drawImage(backgroundImage, 0, 0, width, height);
...
class Shell {
	...
	burst(x, y) {
		...
		if (!this.disableWordd && store.state.config.wordShell) {
			// 假设store.state.config.zizeng第一个是记循环次数，第二个是记索引

			if(store.state.config.zizeng[0]>10){
				let cur = store.state.config.zizeng[1];
				// alert(cur);
				store.state.config.zizeng[1] = (store.state.config.zizeng[1]+1)%randomWords.length;
				store.state.config.zizeng[0]=0;
				createWordBurst(randomWords[cur], dotStarFactory, x, y);
			}else{
				store.state.config.zizeng[0]++;
			}

			// if (Math.random() < 0.1) {
			// 	if (Math.random() < 0.5) {
			// 		let cur = store.state.config.zizeng;
			// 		store.state.config.zizeng = (store.state.config.zizeng+1)%randomWords.length;
			// 		// alert(cur);
			// 		createWordBurst(randomWords[cur], dotStarFactory, x, y);
			// 	}
			// }
